...
static bool channel_processing_check_and_clear(int32_t chan)
{
	if (nrf_rtc_int_enable_check(RTC, RTC_CHANNEL_INT_MASK(chan))) {
		...
		if ((atomic_and(&force_isr_mask, ~BIT(chan)) & BIT(chan)) ||
		    event_check(chan)) {
			if (IS_ENABLED(CONFIG_SOC_NRF53_ANOMALY_165) && (chan == 0)) {
...
void rtc_nrf_isr(const void *arg)
{
	...
	if (nrf_rtc_int_enable_check(RTC, NRF_RTC_INT_OVERFLOW_MASK) &&
		...
		if (IS_ENABLED(CONFIG_SOC_NRF53_ANOMALY_165)) {
			nrf_rtc_event_clear(RTC, RTC_CHANNEL_EVENT_ADDR(ANOMALY_165_OVERFLOW_CHAN));
		}
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	 ...
	 */
	cyc += unannounced;
	cyc = ROUND_UP(cyc, CYC_PER_TICK) * CYC_PER_TICK;
